...
(fn hl [fg bg]
  ...
  {:fg (catppuccin-color (or fg :text))
   :bg (catppuccin-color (or bg :mantle))})
...
(fn get-mode-opts [mode]
  ...
  (let [list [{:modes [:n :niI :niR :niV :nt :nT] :name :NORMAL :color :blue  :icon " "}
              {:modes [:no :nov :noV "no\22"] :name :NORMAL :color :pink :icon "󱦟 "}
              ...
              {:modes [:r :rm :r? :!] :name :... :color :lavender :icon "󰆅 "}
...
(local mode-update {1 :ModeChanged
                    :pattern "*:*"
...
(local mode-bar {:init mode-init
                 ...
                 1 {:provider #(string.format " %s " (. (get-mode-opts $1.mode) :icon))
                    :hl (fn [self] {:fg (catppuccin-color :base)
                                    :bg (let [{: color} (get-mode-opts self.mode)]
                                          (catppuccin-color color 0.2))})}

                 2 {:provider #(string.format " %s " (. (get-mode-opts $1.mode) :name))
                    :hl (fn [self] {:fg (catppuccin-color :base)
                                    :bg (let [{: color} (get-mode-opts self.mode)]
...
(local mode-tag {:init mode-init
                 :update mode-update
                 :provider (fn [] " ")
                 :hl (fn [self] {:bg (let [{: color} (get-mode-opts self.mode)]
                                       (catppuccin-color color 0.2))})})
...
(local git {:condition conds.is_git_repo
            :init (fn [self]
...
(fn lsp-provider [self]
  (let [names []]
    (each [_ {: name} (ipairs (vim.lsp.get_clients {:bufnr 0}))]
      (when (not (some #(= $1 name) self.hidden)) (table.insert names name)))
